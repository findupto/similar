...
function cleanPrinter(p,role){const baud=Number(p?.baudRate)||460800;return {...p,role,name:'YD801',description:'YD801 Bluetooth SPP',bluetoothName:YD801_PROFILE.bluetoothName,bluetoothAddress:YD801_PROFILE.bluetoothAddress,baudRate:baud,profile:YD801_PROFILE,transport:'serial',transportPort:p?.transportPort||p?.port||'',port:p?.transportPort||p?.port||'',printerName:p?.printerName||''}}
async function send(data,p,role='customer'){const printer=cleanPrinter(p,role);if(!printer.port&&!printer.printerName)return{ok:false,error:'No printer port selected'};if(!window.mkPosDesktop?.printEscPos)return{ok:false,error:'Desktop printer service unavailable'};return window.mkPosDesktop.printEscPos({address:printer.bluetoothAddress,port:printer.port,data:encode(data),baudRate:printer.baudRate,printerName:printer.printerName,transport:'serial'})}
export const PrinterBridge={
 async discover(){try{const rows=await window.mkPosDesktop?.discoverPrinters?.()||[];return rows.map(x=>cleanPrinter(x,'customer'))}catch{return[]}},
 ...
 async disconnect(role){const c=cfg();c[role]=null;save(c);return null},
 async test(printer){const p=cleanPrinter(printer||cfg().customer||{},'customer');if(!window.mkPosDesktop?.testSerial)return{ok:false,error:'Desktop printer transport unavailable'};const r=await window.mkPosDesktop.testSerial({address:p.bluetoothAddress,port:p.port,baudRate:p.baudRate,printerName:p.printerName,allPorts:false});const diagnostics=r?.diagnostics||[];const winner=diagnostics.find(x=>x?.ok&&x?.port);if(winner){const updated={...p,port:winner.port,transportPort:winner.port,baudRate:Number(winner.baudRate)||p.baudRate,dtr:!!winner.dtr,rts:!!winner.rts,transport:'serial'};const c=cfg();c.customer=updated;c.lastDiagnostic={...r,selected:updated};save(c);return{...r,selected:updated}}save({...cfg(),lastDiagnostic:r});return r},
 async print(r,p){const c=cfg(),x=cleanPrinter(p||c.customer,'customer');const out=await send(receipt({...r,footer:c.footer}),x,'customer');if(out?.ok)logPrint('customer',r);return out},
 async printCustomer(r,p){return this.print(r,p)},
 async printKitchen(o,p){const c=cfg(),x=cleanPrinter(p||c.kitchen,'kitchen');const out=await send(kitchen({...o,footer:c.kitchenFooter}),x,'kitchen');if(out?.ok)logPrint('kitchen',o);return out},
