...
function createWindow(){
  ...
  win.webContents.on('did-fail-load',(_e,errorCode,errorDescription,validatedURL)=>console.error('[MK POS] did-fail-load',errorCode,errorDescription,validatedURL));
  win.webContents.on('render-process-gone',(_e,details)=>console.error('[MK POS] render-process-gone',details));
  win.webContents.on('console-message',(_e,level,message,line,source)=>{if(level>=2)console.error('[MK POS renderer]',message,source+':'+line)});
  if(isDev)loadDevUrl();else win.loadURL(pathToFileURL(path.join(__dirname,'..','dist','index.html')).href).catch(err=>console.error('[MK POS] packaged load failed',err.message));
}
...
ipcMain.handle('printer:print',async(_e,{address,data,port,baudRate=9600,printerName,transport='serial',dtr=false,rts=false}={})=>{if(printerName){const q=await queueSend(printerName,data);if(q.ok)return q;if(transport==='windows-raw')return q}const selected=safePort(port);if(selected)return serialSend(selected,data,baudRate,{dtr,rts});return{ok:false,transport:'none',bluetoothAddress:address||'',error:'No selected YD801 COM port and Windows printer queue failed'}});
